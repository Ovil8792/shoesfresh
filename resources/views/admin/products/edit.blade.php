    ...
    <div class="row mb-4">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="text-muted mb-0">Chỉnh sửa thông tin sản phẩm và biến thể</p>
            </div>
        ...
        <div class="row">
            <div class="col-lg-8">
                ...
                <div class="card border-0 shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <p class="text-muted small">Quản lý các biến thể khác nhau của sản phẩm</p>
                        <div id="variantsContainer">
                            @if(isset($variants) && $variants->count() > 0)
                                @foreach($variants as $index => $variant)
                                    @include('admin.products._variant_item', ['variant' => $variant, 'index' => $index])
                                @endforeach
                            @endif
                        </div>
                    </div>
                ...
                <div class="card border-0 shadow-sm">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            @if($product->image)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $product->image) }}" alt="Current image" class="img-thumbnail" style="max-width: 100%; max-height: 200px;">
                                </div>
                            @endif
...
<script>
let variantIndex = {{ isset($variants) && $variants->count() > 0 ? $variants->count() : 0 }};
...
document.addEventListener('click', function(e) {
    if (e.target.closest('.removeVariant')) {
        ...
        const variantId = variantItem.querySelector('input[name*="[id]"]');
